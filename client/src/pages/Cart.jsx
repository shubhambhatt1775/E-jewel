...
const generateInvoice = (order) => {
  ...
  const doc = new jsPDF();
  const { id, items, address, totalAmount, userName, date } = order;

  // ===== LOGO =====
  // If you have a logo, put it in /public/logo.png
  const logo = logos;
  // Uncomment if you want to use logo
  doc.addImage(logo, "PNG", 14, 10, 30, 15);

  // ===== HEADER =====
  doc.setFontSize(18);
  doc.text("E-JEWEL", 50, 20);
  doc.setFontSize(14);
  doc.text("INVOICE", 160, 20);

  // ===== CUSTOMER INFO =====
  doc.setFontSize(11);
  doc.text(`Order ID: ${id}`, 14, 40);
  doc.text(`Date: ${date}`, 14, 46);
  doc.text(`Customer: ${userName}`, 14, 52);

  doc.text(`Shipping Address:`, 14, 60);
  doc.text(`${address.street}, ${address.city}`, 14, 66);
  doc.text(`${address.state}, ${address.country}`, 14, 72);
            ...
            if(paymentOption === "COD"){
                ...
                if(data.success){
                    ...
                    const orderForInvoice = {
                      ...
                      totalAmount: getCartAmount() + (getCartAmount() * 2 / 100), // including tax
    ...
    return products.length > 0 && cartItems ? (
        <div className="flex flex-col md:flex-row mt-16">
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                //                     navigate("/card", {
                //                     state: {
                //                     value: { cardNumber: "", cardHolder: "", month: "", year: "", cvv: "" }
                //                                }
                //                     });
